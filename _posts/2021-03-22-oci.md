---
title: Устнавка Arch Linux в Oracle Cloud
description: Разворачивание archbotstrap на виртуальном сервере oracle cloud
modified: 2021-03-22
tags: Linux
---

Существует такое предложение от [Oracle Cloud](https://www.oracle.com/cloud/free/), можно получить 2 сервера в минимальной комплектации, на всегда и первые 30 дней тестирования более широкого функционала, нужно привязать карту с которой спишут примерно 1 евро(мне вернули практически сразу) и телефон. Доступные дистрибутивы на free тарифе: ubuntu, centos, oracle linux.
...
Отмантировать swap он 8G, форматировать его в нужную фс(btrfs/ext4). В моем случае структура разделов.
...
```bash
pacstrap /mnt base base-devel linux-lts linux-lts-headers btrfs-progs nano grub efibootmgr iproute2 iputils rng-tools open-iscsi arch-install-scripts pacman-contrib reflector openssh wget git rsync gnu-netcat pv unzip unrar p7zip zsh htop tmux
...
Переустнавим и сконфигурируем grub.
...
Выходим из chroot, отмантируем, ребут.
...
Это полезно при обновлении ядра, можно очень быстро перезагрузить его, не дожидаясь завершения всего процесса загрузки BIOS. [wiki.archlinux.org](https://wiki.archlinux.org/index.php/Kexec) Действие нужно смотреть когда доступно обновление ядра, думаю понятно.
